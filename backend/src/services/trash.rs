...
pub async fn cleanup_trash(
    ...
    // 查找过期的文件
    let expired_files: Vec<MediaFile> = sqlx::query_as(
        "SELECT * FROM media_files WHERE path LIKE ? AND updated_at < ?"
    )
...
pub async fn list_trash(
    ...
    let files: Vec<MediaFile> = sqlx::query_as(
        "SELECT * FROM media_files WHERE path LIKE ? ORDER BY updated_at DESC"
    )
    .bind(format!("{}%", trash_prefix))
    .fetch_all(db)
    .await?;

    let trash_items: Vec<TrashItem> = files.into_iter().map(|file| {
        TrashItem {
            id: file.id,
