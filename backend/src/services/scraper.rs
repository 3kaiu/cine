...
pub fn parse_filename(filename: &str) -> (String, Option<u32>, Option<u32>, Option<u32>) {
    ...
    let year = year_re.find(&name)
        .and_then(|m| m.as_str().parse::<u32>().ok());

    // 提取剧集信息：S01E01, S1E1, 第1集, EP01
    let season_episode_re = Regex::new(r"(?:S|Season|第)(\d+)(?:E|Episode|集)(\d+)").unwrap();
    let (season, episode) = if let Some(caps) = season_episode_re.captures(&name) {
        ...
        // 尝试其他格式：EP01, E01
        let ep_re = Regex::new(r"EP?(\d+)").unwrap();
        let episode = ep_re.captures(&name)
...
pub async fn search_movie_tmdb(
    ...
    for result in results {
        let movie = MovieMetadata {
            ...
            backdrop_url: result.get("backdrop_path")
                .and_then(|b| b.as_str())
...
pub async fn scrape_metadata(
    ...
    if is_tv_show {
        // 搜索剧集
        let shows = search_tv_tmdb(&title, year, api_key).await?;
        ...
        // 搜索电影
        let movies = search_movie_tmdb(&title, year, api_key).await?;
...
pub async fn batch_scrape_metadata(
    ...
    for file in files {
        let result = match scrape_metadata(file, source, auto_match, config).await {
            Ok(metadata) => {
                // 如果成功，下载图片和生成 NFO
                if download_images || generate_nfo {
                    let poster_url = metadata.get("poster_url")
                        .and_then(|u| u.as_str());
                    let backdrop_url = metadata.get("backdrop_url")
                        .and_then(|u| u.as_str());
